...
#num_train = 1200
dataset_name = 'MSTAR-10'
data_folder = '../datas/' + dataset_name + '/'
num_train = 6
...
class MstarTask(object):
    ...
    def __init__(self, character_folders, num_classes, train_num,test_num, phase = 'training'):
        ...
        #        temp.sort()
#
        #        self.train_roots += temp[:train_num]
        #        remains = temp[train_num : len(temp)]
        #        self.test_roots += random.sample(remains, test_num)
...
class Mstar(FewShotDataset):

    def __init__(self, *args, **kwargs):
        super(Mstar, self).__init__(*args, **kwargs)
...
def get_data_loader(task, num_per_class=1, split='train',shuffle=True,rotation=0):
    ...
    dataset = Mstar(task, split=split, transform=transforms.Compose([Rotate(rotation), transforms.ToTensor(), normalize]))
